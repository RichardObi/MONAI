...
from monai.apps.medigan_dataset import MediganDataset
from monai.data import MetaTensor
from monai.transforms import AddChanneld, Compose, LoadImaged, ScaleIntensityd
from tests.utils import skip_if_downloading_fails, skip_if_quick

test_arguments = [{'model_id': 1, 'num_samples': 10},
                     {'model_id': 2, 'num_samples': 20},
                     {'model_id': 3, 'num_samples': 30},
                     {'model_id': 4, 'num_samples': 40},
                     {'model_id': 5, 'num_samples': 50}]


class TestMediganDataset(unittest.TestCase):
    @skip_if_quick
